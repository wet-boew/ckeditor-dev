...
(function() {
	...
	function divDialog( editor, command ) {
		...
		function createPar( editor ) {
			...
			for ( i = 0; i < blockGroups.length; i++ ) {
				...
				divElement = new CKEDITOR.dom.element( 'p', editor.document );
		...
		function commitInternally( targetFields ) {
			var dialog = this.getDialog(),
				element = dialog._element && dialog._element.clone() || new CKEDITOR.dom.element( 'p', editor.document );
		...
		return {
			...
			contents: [
				...
				elements: [
					...
					children: [
						...
						multiple: true,
						size: 10,
						label: editor.lang.par.styleSelectLabel,
						'default': '',
				...
				editor.getStylesSet( function( stylesDefinitions ) {
					...
					if ( stylesDefinitions ) {
						...
						for ( var i = 0; i < stylesDefinitions.length; i++ ) {
							...
							if ( styleDefinition.element && styleDefinition.element == 'p' ) {
			...
			onShow: function() {
				...
				if ( command == 'editpar' ) {
					...
					this.setupContent( this._element = CKEDITOR.plugins.par.getSurroundPar( editor ) );
				}
			},
			onOk: function() {
				if ( command == 'editpar' )
					...
					containers = createPar( editor, true );
	...
	CKEDITOR.dialog.add( 'createpar', function( editor ) {
		return divDialog( editor, 'createpar' );
	});
	CKEDITOR.dialog.add( 'editpar', function( editor ) {
		return divDialog( editor, 'editpar' );
	});
})();
